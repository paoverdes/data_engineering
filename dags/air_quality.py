...
def insert_information(data_all_sites):
    ...
    for i in range(len(df)):
        ...
        key = (region + '-' + str(date.today())).strip()
        conn.execute("insert into airquality_data_temp (key, region, aqi, co, no2, o3, status, date_info, execution_date) "
                       "values (%s, %s, %s, %s, %s, %s, %s, %s, current_date)",
                       (key, region, aqi, co, no2, o3, 'SUCCESS', date_info))
        conn.execute("DELETE FROM airquality_data_temp "
                     "WHERE id IN ("
                     "SELECT id FROM ( SELECT id, "
                     "ROW_NUMBER() OVER (PARTITION BY key, region) AS rn "
                     "FROM airquality_data_temp) sub "
                     "WHERE rn > 1);")
        conn.execute("merge into airquality_data "
                       ...
                       "date_info = temp.date_info, execution_date = temp.execution_date "
                       "WHEN NOT MATCHED THEN INSERT (key, region, aqi, co, no2, o3, status, date_info, execution_date) "
                       "values (%s, %s, %s, %s, %s, %s, %s, %s, current_date) ",
                       (key, region, aqi, co, no2, o3, 'SUCCESS', date_info))
        conn.execute("delete from airquality_data_temp;")
    conn.close()
...
def verify_information():
    conn = new_db_connection()
    query = 'select distinct region , aqi, date_info, level, min_val, max_val ' \
            'from airquality_data ad join airquality_index ai ' \
            'on ad.aqi >= ai.min_val ' \
            'and ad.aqi <= ai.max_val where DATE(ad.date_info) = current_date; '
